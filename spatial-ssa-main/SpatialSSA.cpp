...
int main(int argc, char** argv) {
    ...
    // // Doesn't work in current implementation any more.
    // // Model assumes wrong things about energies to work with square grid
    ...
    // model_ptr->load_phase_separation_parameters();
    ...
    // model_ptr->n_iterations = 5000000;


    // ----------------------------------------------------------------------------------------------------------------
    // --------------------------------------- ---Yeast like phase separation------------------------------------------
    // ----------------------------------------------------------------------------------------------------------------
    ...
    // model_ptr->load_model("./Models/diffusion_model.txt");
    // model_ptr->load_initial_conditions("./Models/diffusion_setup_yeast_like.txt");
    ...
    model_ptr->crowding       = false;

    // in-depth print-outs
    model_ptr->verbose = false;
    model_ptr->timing = false;

    model_ptr->record_indv_cells = false;
    model_ptr->record_propensities = false;
    
    // Simulate 10 seconds, record every 5000 iterations.
    model_ptr->t_max        = 2562;
    model_ptr->break_iter   = 5000;
    model_ptr->n_iterations = 6000000; // 2 million for 2562
    ...
    for(int i_variable = 0; i_variable < n_variables; i_variable++){
        ...
        cout << "Adding variables to parameter dictionary" << endl;
    ...
    thread.launch();
    //std::thread thread (&run, model_ptr);
    
    // render(model_ptr, false, false);
